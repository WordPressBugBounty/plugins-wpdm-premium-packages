...
function wpdmpp_load_saved_cart(){
    if( isset( $_REQUEST['savedcart'] ) ) {
        ...
        $saved_cart_data = \WPDM\__\Crypt::decrypt($saved_cart_data, true);
        ...
        if(is_array($saved_cart_data) && count($saved_cart_data) > 0) {
            ...
            if($coupon_data){
                ...
                //update_option($cart_id."_coupon", $coupon_data, false);
                TempStorage::set($cart_id."_coupon", $coupon_data, 10800);
            }
        }
    ...
    if( isset( $_REQUEST['addtocart'])) {
        ...
        if(WPDMPP()->cart->isLocked()) {
	        ...
	        if(__::is_ajax())
                ...
                Messages::fullPage(__('Cart is locked', WPDMPP_TEXT_DOMAIN), UI::card(__('Cart is locked', WPDMPP_TEXT_DOMAIN), [$message]));*/
        ...
        do_action("wpdmpp_after_addtocart", $cart_data);
...
function wpdmpp_remove_cart_item(){
    ...
    if( !isset($_REQUEST['wpdmpp_remove_cart_item'])) return;
    $cart_data = wpdmpp_get_cart_data();
    $rciid = $_REQUEST['wpdmpp_remove_cart_item'];
    if($rciid === 'all') {
        wpdmpp_empty_cart();
        $ret = ['success' => true];
    }
    else {
        WPDMPP()->cart->removeItem($rciid);
        if(!count(WPDMPP()->cart->getItems()))
            WPDMPP()->cart->clear();
        else {
	        WPDMPP()->cart->clearCoupon();
        }
        $ret = ['success' => true, 'refresh' => true];
    }
...
function wpdmpp_update_cart(){
    ...
    if(wpdm_query_var('coupon_code') != ''){
        $discount = CouponCodes::validate_coupon(wpdm_query_var('coupon_code'));
...
function wpdmpp_update_cart_data($cart_data){
    $cart_id = wpdmpp_cart_id();
    $ret = update_option($cart_id, $cart_data, false);
    return $ret;
}
...
function wpdmpp_get_cart_discount(){
    $current_user = wp_get_current_user();
    $role = is_user_logged_in() ? array_shift($current_user->roles) : 'guest';
    $cart_items = wpdmpp_get_cart_items();
    $discount_r = 0;
...
function wpdmpp_get_all_coupon( $data ){
    ...
    foreach($data as $pid => $item){
        $valid_coupon = isset($item['coupon']) ? wpdmpp_check_coupon($pid, $item['coupon'], $data) : 0;
...
function wpdmpp_product_price_html($product_id){
    ...
    ob_start();
    include Template::locate("add-to-cart/price.php", WPDMPP_TPL_DIR);
    return ob_get_clean();
 ...
 * @return array
 */
function wpdmpp_product_license_options($product_id){
...
function wpdmpp_product_license_options_html($product_id){
    ...
    ob_start();
    include Template::locate("add-to-cart/select-license.php", WPDMPP_TPL_DIR);
    return ob_get_clean();
}

function wpdmpp_product_gigs_options_html($product_id){
    return include(Template::locate("add-to-cart/extra-gigs.php", WPDMPP_TPL_DIR));
}
...
function wpdmpp_add_to_cart_form( $product_id , $template = ''){
    ...
    if((int)get_post_meta($product_id, '__wpdm_pay_as_you_want', true) == 1)
        include Template::locate("add-to-cart/pay-as-you-want-form.php", WPDMPP_TPL_DIR);
    else
        include Template::locate("add-to-cart/form.php", WPDMPP_TPL_DIR);
...
function wpdmpp_waytocart($product, $btnclass = 'btn-info'){
    ...
    $license_enabled = get_post_meta($product['ID'], "__wpdm_enable_license", true);
