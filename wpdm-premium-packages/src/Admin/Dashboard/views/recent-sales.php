...
// Pre-fetch all posts in a single query to avoid N+1
$pids = [];
        ...
        <div class="wpdmpp-recent-list">
            ...
            <?php else: ?>
                <?php foreach ($latest_items as $item):
                    // Use pre-fetched post data (no individual DB queries)
                    $post_info = isset($posts_data[$item->pid]) ? $posts_data[$item->pid] : null;
