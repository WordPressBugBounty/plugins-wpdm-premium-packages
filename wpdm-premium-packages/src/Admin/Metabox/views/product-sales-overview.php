...
// Check cache first - keyed by product ID and current hour (refreshes hourly)
$cache_key = 'wpdmpp_product_sales_' . $pid . '_' . wp_date( 'Y-m-d-H' );
...
if ( false === $sales ) {
    ...
    if ( $sales ) {
        set_transient( $cache_key, $sales, 30 * MINUTE_IN_SECONDS );
    }
}
