...
<div class="wrap">
    ...
    <div class="tab-content">
        ...
        <section class="tab-pane in <?= wpdm_query_var('ppstab', 'txt') === 'ppcurrency' ? 'active' : '' ?>" id="ppcurrency">
            <?php include_once("currency-options.php"); ?>
        </section>
...
<script>
    jQuery(function($){
        ...
        $('.wpdmpp-settings-tabs__link').on('click', function(e) {
            ...
            }, "", "edit.php?post_type=wpdmpro&page=<?= $settings_page ?>&tab=ppsettings&ppstab=" + tabId);

            // Trigger events for compatibility and re-initialization
            $this.trigger('shown.bs.tab');
            $(document).trigger('wpdmpp_settings_tab_shown', [tabId]);

            // Re-initialize payment options when payment tab is shown
            if (tabId === 'pppayment' && typeof window.wpdmppInitPaymentOptions === 'function') {
                setTimeout(window.wpdmppInitPaymentOptions, 50);
            }
        });
        ...
        // Initialize payment options if payment tab is already active on load
        var activeTab = $('.wpdmpp-settings-tabs__link.active').data('pptab');
        if (activeTab === 'pppayment' && typeof window.wpdmppInitPaymentOptions === 'function') {
            setTimeout(window.wpdmppInitPaymentOptions, 100);
        }
    });
</script>
