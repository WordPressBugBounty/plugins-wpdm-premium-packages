...
<div class="wpdmpp-settings-section">
    ...
    <div class="wpdmpp-card">
        ...
        <div class="wpdmpp-card__body">
            ...
            <!-- Order Settings -->
            <div class="wpdmpp-toggle-option" style="flex-direction: column; align-items: stretch; gap: 12px;">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div class="wpdmpp-toggle-option__info" style="padding-right: 0;">
                        <h4 class="wpdmpp-toggle-option__label"><?php _e('Order Validity Period', 'wpdm-premium-packages'); ?></h4>
                        <p class="wpdmpp-toggle-option__desc"><?php _e('How long customers can access purchased downloads after completing an order. After this period, the order expires and downloads become unavailable unless renewed.', 'wpdm-premium-packages'); ?></p>
                    </div>
                </div>
                <div style="display: flex; align-items: center; gap: 8px;">
                    <?php
                    $validity_days = isset($settings['order_validity_period']) ? (int) $settings['order_validity_period'] : 365;
                    // Determine best unit for display
                    if ($validity_days > 0 && $validity_days % 365 === 0) {
                        $display_value = $validity_days / 365;
                        $display_unit = 'years';
                    } elseif ($validity_days > 0 && $validity_days % 30 === 0) {
                        $display_value = $validity_days / 30;
                        $display_unit = 'months';
                    } else {
                        $display_value = $validity_days;
                        $display_unit = 'days';
                    }
                    ?>
                    <input type="number" min="0" step="1" class="form-control" style="width: 90px;" id="wpdmpp_validity_display" value="<?php echo esc_attr($display_value); ?>">
                    <select class="form-control" style="width: 120px;" id="wpdmpp_validity_unit">
                        <option value="days" <?php selected($display_unit, 'days'); ?>><?php _e('Days', 'wpdm-premium-packages'); ?></option>
                        <option value="months" <?php selected($display_unit, 'months'); ?>><?php _e('Months', 'wpdm-premium-packages'); ?></option>
                        <option value="years" <?php selected($display_unit, 'years'); ?>><?php _e('Years', 'wpdm-premium-packages'); ?></option>
                    </select>
                    <input type="hidden" name="_wpdmpp_settings[order_validity_period]" id="wpdmpp_validity_days" value="<?php echo esc_attr($validity_days); ?>">
                    <span style="color: #94a3b8; font-size: 12px;" id="wpdmpp_validity_hint">(<?php echo esc_html($validity_days); ?> <?php _e('days', 'wpdm-premium-packages'); ?>)</span>
                </div>
            </div>
            <script>
            jQuery(function($) {
                var $display = $('#wpdmpp_validity_display'),
                    $unit = $('#wpdmpp_validity_unit'),
                    $hidden = $('#wpdmpp_validity_days'),
                    $hint = $('#wpdmpp_validity_hint');
                var multipliers = { days: 1, months: 30, years: 365 };

                function updateDays() {
                    var val = parseInt($display.val()) || 0;
                    var days = val * multipliers[$unit.val()];
                    $hidden.val(days);
                    $hint.text('(' + days + ' <?php echo esc_js(__('days', 'wpdm-premium-packages')); ?>)');
                }

                $display.on('input change', updateDays);
                $unit.on('change', updateDays);
            });
            </script>
    ...
    <div class="wpdmpp-card">
        <div class="wpdmpp-card__header">
            ...
            <div>
                <h3 class="wpdmpp-card__title"><?php _e('Error Logging', 'wpdm-premium-packages'); ?></h3>
                <div class="wpdmpp-card__subtitle"><?php _e('Configure error logging for debugging and troubleshooting', 'wpdm-premium-packages'); ?></div>
            </div>
        </div>
        <div class="wpdmpp-card__body">
            <div class="wpdmpp-toggle-option" style="margin-bottom: 20px;">
                <div class="wpdmpp-toggle-option__info">
                    <h4 class="wpdmpp-toggle-option__label"><?php _e('Enable Error Logging', 'wpdm-premium-packages'); ?></h4>
                    <p class="wpdmpp-toggle-option__desc"><?php _e('Log errors, warnings, and important events to a file for debugging', 'wpdm-premium-packages'); ?></p>
                </div>
                <label class="wpdmpp-switch">
                    <input type="hidden" name="_wpdmpp_settings[enable_error_logging]" value="0">
                    <input type="checkbox" name="_wpdmpp_settings[enable_error_logging]" value="1" <?php checked(get_wpdmpp_option('enable_error_logging', 1)); ?>>
                    <span class="wpdmpp-switch__slider"></span>
                </label>
            </div>

            <div class="wpdmpp-form-group">
                <label class="wpdmpp-form-group__label"><?php _e('Log Level', 'wpdm-premium-packages'); ?></label>
                <select class="form-control" name="_wpdmpp_settings[log_level]" style="max-width: 300px;">
                    <?php
                    $log_levels = [
                        'emergency' => __('Emergency - System is unusable', 'wpdm-premium-packages'),
                        'alert'     => __('Alert - Immediate action required', 'wpdm-premium-packages'),
                        'critical'  => __('Critical - Critical conditions', 'wpdm-premium-packages'),
                        'error'     => __('Error - Runtime errors (Recommended)', 'wpdm-premium-packages'),
                        'warning'   => __('Warning - Exceptional occurrences', 'wpdm-premium-packages'),
                        'notice'    => __('Notice - Normal but significant events', 'wpdm-premium-packages'),
                        'info'      => __('Info - Interesting events', 'wpdm-premium-packages'),
                        'debug'     => __('Debug - Detailed debug information', 'wpdm-premium-packages'),
                    ];
                    $current_level = get_wpdmpp_option('log_level', 'error');
                    foreach ($log_levels as $level => $label):
                    ?>
                    <option value="<?php echo esc_attr($level); ?>" <?php selected($current_level, $level); ?>><?php echo esc_html($label); ?></option>
                    <?php endforeach; ?>
                </select>
                <p class="wpdmpp-form-group__hint"><?php _e('Only messages at this level or more severe will be logged', 'wpdm-premium-packages'); ?></p>
            </div>

            <?php
            // Show log files if they exist
            if (class_exists('\\WPDMPP\\Libs\\Logger')) {
                $log_files = \WPDMPP\Libs\Logger::get_log_files();
                if (!empty($log_files)):
            ?>
            <div class="wpdmpp-divider"></div>
            <div class="wpdmpp-form-group">
                <label class="wpdmpp-form-group__label"><?php _e('Log Files', 'wpdm-premium-packages'); ?></label>
                <div class="wpdmpp-log-files" style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px;">
                    ...
                    <div style="display: flex; align-items: center; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #e2e8f0;">
                        <div>
                            <span style="font-family: monospace; font-size: 13px; color: #1e293b;"><?php echo esc_html($log['name']); ?></span>
                ...
                <p class="wpdmpp-form-group__hint" style="margin-top: 8px;">
                    <?php printf(
                        __('Log files are stored in: %s', 'wpdm-premium-packages'),
                        '<code style="font-size: 11px;">' . esc_html(\WPDMPP\Libs\Logger::get_log_dir()) . '</code>'
                    ); ?>
                </p>
                <button type="button" class="button button-secondary" id="wpdmpp-clear-logs" style="margin-top: 8px;">
                    <?php _e('Clear All Logs', 'wpdm-premium-packages'); ?>
                </button>
            </div>
            <?php
                endif;
...
jQuery(function ($) {
    ...
    $('#wpdmpp-clear-logs').on('click', function() {
        if (!confirm('<?php echo esc_js(__('Are you sure you want to delete all log files?', 'wpdm-premium-packages')); ?>')) {
            return;
        }

        var $btn = $(this);
        $btn.prop('disabled', true).text('<?php echo esc_js(__('Clearing...', 'wpdm-premium-packages')); ?>');

        $.post(ajaxurl, {
            action: 'wpdmpp_clear_logs',
            _wpnonce: '<?php echo wp_create_nonce('wpdmpp_clear_logs'); ?>'
        }, function(response) {
            if (response.success) {
                location.reload();
            } else {
                alert(response.data || '<?php echo esc_js(__('Failed to clear logs', 'wpdm-premium-packages')); ?>');
                $btn.prop('disabled', false).text('<?php echo esc_js(__('Clear All Logs', 'wpdm-premium-packages')); ?>');
            }
        });
    });
});
