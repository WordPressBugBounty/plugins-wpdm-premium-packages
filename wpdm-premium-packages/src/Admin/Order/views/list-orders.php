...
if ( wpdm_query_var( 'oid', 'txt' ) != '' ) {
	$qry[] = $wpdb->prepare( "order_id = %s", wpdm_query_var( 'oid', 'txt' ) );
}
if ( wpdm_query_var( 'customer', 'txt' ) != '' ) {
	...
	$qry[] = $wpdb->prepare( "uid = %d", absint( $customer ) );
}
if ( wpdm_query_var( 'ost', 'txt' ) != 'Expiring' ) {
	if ( wpdm_query_var( 'ost', 'txt' ) != '' ) {
		$qry[] = $wpdb->prepare( "order_status = %s", wpdm_query_var( 'ost', 'txt' ) );
	}
	if ( wpdm_query_var( 'pst', 'txt' ) != '' ) {
		$qry[] = $wpdb->prepare( "payment_status = %s", wpdm_query_var( 'pst', 'txt' ) );
	}
	...
	if ( $sdate_input != '' || $edate_input != '' ) {
		...
		if ( $sdate && $edate ) {
			$qry[] = $wpdb->prepare( "(`date` >= %d AND `date` <= %d)", $sdate, $edate );
		}
	}
} else {
	...
	$qry[] = $wpdb->prepare( "(`expire_date` >= %d AND `expire_date` <= %d)", $sdate, $edate );
}
...
$allowed_orderby = array( 'date', 'expire_date', 'order_id', 'total', 'uid', 'order_status', 'payment_status' );
$orderby_input = wpdm_query_var( 'orderby', 'txt' );
if ( $orderby_input != '' && in_array( $orderby_input, $allowed_orderby, true ) ) {
	$orderby = $orderby_input;
	$_order  = wpdm_query_var( 'order', 'txt' ) === 'asc' ? 'ASC' : 'DESC';
...
// Any active filter? The global stats cards/totals don't reflect a filtered
// result set, so they are hidden whenever a filter is applied.
$has_filter = wpdm_query_var( 'oid', 'txt' ) !== ''
	|| wpdm_query_var( 'customer', 'txt' ) !== ''
	|| wpdm_query_var( 'ost', 'txt' ) !== ''
	|| wpdm_query_var( 'pst', 'txt' ) !== ''
	|| wpdm_query_var( 'sdate', 'txt' ) !== ''
	|| wpdm_query_var( 'edate', 'txt' ) !== '';
...
if(!wpdm_query_var('customer', 'txt') && !wpdm_query_var('oid', 'txt')) {
	// Cache key based on current date (refreshes daily at minimum)
	$cache_key = 'wpdmpp_order_stats_' . wp_date( 'Y-m-d-H' );
	$stats = get_transient( $cache_key );

	if ( false === $stats ) {
		$sdatet = strtotime( wp_date( "Y-m-d" ) . " 00:00:00" );
		$edatet = strtotime( wp_date( "Y-m-d" ) . " 23:59:59" );

		// Single optimized query for all order stats (replaces 5 separate queries)
		$order_stats = $wpdb->get_row( $wpdb->prepare( "
			SELECT
				SUM(CASE WHEN payment_status IN ('Completed', 'Expired') THEN total ELSE 0 END) AS completed_sales,
				SUM(CASE WHEN payment_status IN ('Completed', 'Expired') THEN 1 ELSE 0 END) AS completed_orders,
				SUM(CASE WHEN payment_status = 'Expired' THEN total ELSE 0 END) AS expired_sales,
				SUM(CASE WHEN payment_status = 'Expired' THEN 1 ELSE 0 END) AS expired_orders,
				SUM(CASE WHEN payment_status = 'Refunded' THEN total ELSE 0 END) AS refunded_sales,
				SUM(CASE WHEN payment_status = 'Refunded' THEN 1 ELSE 0 END) AS refunded_orders,
				SUM(CASE WHEN payment_status = 'Processing' THEN total ELSE 0 END) AS abandoned_sales,
				SUM(CASE WHEN payment_status = 'Processing' THEN 1 ELSE 0 END) AS abandoned_orders,
				SUM(CASE WHEN payment_status = 'Completed' AND `date` >= %d AND `date` <= %d THEN total ELSE 0 END) AS newtoday_sales,
				SUM(CASE WHEN payment_status = 'Completed' AND `date` >= %d AND `date` <= %d THEN 1 ELSE 0 END) AS newtoday_orders
			FROM {$wpdb->prefix}ahm_orders
		", $sdatet, $edatet, $sdatet, $edatet ) );

		// Separate query for renews (different table)
		$renew_stats = $wpdb->get_row( $wpdb->prepare( "
			SELECT
				SUM(base_total) AS allrenews_sales,
				COUNT(total) AS allrenews_orders,
				SUM(CASE WHEN `date` >= %d AND `date` <= %d THEN total ELSE 0 END) AS renewtoday_sales,
				SUM(CASE WHEN `date` >= %d AND `date` <= %d THEN 1 ELSE 0 END) AS renewtoday_orders
			FROM {$wpdb->prefix}ahm_order_renews
		", $sdatet, $edatet, $sdatet, $edatet ) );

		// Structure results for compatibility
		$stats = array(
			'completed'  => (object) array( 'sales' => $order_stats->completed_sales, 'orders' => $order_stats->completed_orders ),
			'expired'    => (object) array( 'sales' => $order_stats->expired_sales, 'orders' => $order_stats->expired_orders ),
			'refunded'   => (object) array( 'sales' => $order_stats->refunded_sales, 'orders' => $order_stats->refunded_orders ),
			'abandoned'  => (object) array( 'sales' => $order_stats->abandoned_sales, 'orders' => $order_stats->abandoned_orders ),
			'allrenews'  => (object) array( 'sales' => $renew_stats->allrenews_sales, 'orders' => $renew_stats->allrenews_orders ),
			'newtoday'   => (object) array( 'sales' => $order_stats->newtoday_sales, 'orders' => $order_stats->newtoday_orders ),
			'renewtoday' => (object) array( 'sales' => $renew_stats->renewtoday_sales, 'orders' => $renew_stats->renewtoday_orders ),
		);
	...
	// Extract for template compatibility
	$completed  = $stats['completed'];
	$expired    = $stats['expired'];
	$refunded   = $stats['refunded'];
	$abandoned  = $stats['abandoned'];
	$allrenews  = $stats['allrenews'];
	$newtoday   = $stats['newtoday'];
	$renewtoday = $stats['renewtoday'];
}
...
<style>
/* ==========================================================================
   Orders list — enterprise admin surface. Scoped under .wpdmpp-ol so it never
   leaks into other admin screens. Tokens fall back to literals so the design
   holds even where the WPDM theme variables aren't defined.
   ========================================================================== */
.wpdmpp-ol{
...
/* The jQuery UI datepicker is appended to <body> and gets a z-index computed from
   the input's stacking context (near 0 here), so it renders behind the table. Force
   it above the list. !important beats the inline z-index jQuery UI sets on the widget. */
...
/* per-metric tones: icon tile + status dot */
...
/* result meta bar (inside #order-search, swapped on AJAX search) */
.wpdmpp-ol__resultbar{
	display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;
	padding:11px 18px;border-top:1px solid var(--ol-border-soft);
	background:var(--ol-surface-2);
	font-size:13px;color:var(--ol-muted);
}
...
/* --- Table --------------------------------------------------------------- */
/* Higher specificity than .panel.panel-default so its overflow:hidden (for rounded
   corners) doesn't clip the wide table — the right-hand columns scroll instead. */
...
.wpdmpp-ol table.table-wpdmpp{width:100%;min-width:1040px;border-collapse:separate;border-spacing:0;margin:0;background:var(--ol-surface);font-variant-numeric:tabular-nums;}
.wpdmpp-ol table.table-wpdmpp thead th{
...
.wpdmpp-ol table.table-wpdmpp tbody td{
	padding:13px 14px;vertical-align:middle;
	color:var(--ol-text);font-size:13.5px;background:transparent;
}
...
/* compact payment-status indicator beside the total (replaces the Payment column) */
...
.wpdmpp-ol .ol-date__t{font-size:11.5px;color:var(--ol-faint);white-space:nowrap;}
.wpdmpp-ol .ol-muted{color:var(--ol-muted);}

/* status pills (icon + label) */
.wpdmpp-ol .ol-pill{
...
/* boolean indicator dots (downloaded / auto-renew) */
...
/* auto-renew toggle badge — class swapped live by the toggle JS in orders.php */
...
.wpdmpp-ol .tablenav-pages .page-numbers{
	display:inline-flex;align-items:center;justify-content:center;
	min-width:32px;height:32px;padding:0 9px;margin:0;
...
/* ==========================================================================
   Mobile — swap the table for Vue-rendered cards (preserves .wpmo-* + Vue).
   ========================================================================== */
#wpdmpp-orders-mobile{display:none;}
...
@media screen and (max-width:782px){
	...
	/* Drop the desktop min-width so the stacked fallback table isn't forced wide. */
	.wpdmpp-ol table.table-wpdmpp{min-width:0;}
	.wpdmpp-orders-responsive.is-vue-ready .wpdmpp-orders-tablewrap{display:none;}
	.wpdmpp-orders-responsive.is-vue-ready #wpdmpp-orders-mobile{display:block;}
	/* Fallback when Vue is unavailable: stacked table */
	...
	<form method="get" action="" id="order-search">
		...
		<div class="panel panel-default">
			...
			<div class="wpdmpp-ol__resultbar">
				<span class="ol-count"><?php echo number_format_i18n($t); ?> <?php _e("order(s) found","wpdm-premium-packages");?></span>
				<?php if(!$has_filter) { // Global completed-sales total only makes sense unfiltered. ?>
	...
	<form method="get" action="<?php echo admin_url('/edit.php'); ?>" id="orders-form">
		...
		<div class="panel panel-default wpdmpp-orders-tablewrap">
			<table cellspacing="0" class="table table-striped table-wpdmpp">
				<thead>
				<tr>
					<th style="width: 42px" class="manage-column column-cb check-column" scope="col"><input type="checkbox" aria-label="<?php esc_attr_e('Select all orders','wpdm-premium-packages'); ?>"></th>
					...
					<th class="manage-column" scope="col"><a href="edit.php?post_type=wpdmpro&page=orders&paged=<?php echo wpdm_query_var('paged') ?>&orderby=expire_date&order=<?php echo wpdm_query_var('order') == 'asc'?'desc':'asc';  ?>"><?php _e("Expire Date","wpdm-premium-packages");?> <?php if(wpdm_query_var('order') == 'desc') echo '&#9660;'; if(wpdm_query_var('order') == 'asc') echo '&#9650;';  ?></a></th>
				...
				foreach($orders as $order) {
					...
					$mobile_orders[] = array(
						...
						'expire_date'    => wp_date( get_option('date_format') . ' ' . get_option('time_format'), $order->expire_date ),
					...
					<tr class="<?php echo wpdm_query_var('focus') === $order->order_id ? 'row-focus' : '' ?>">
						<th class="check-column" scope="row" data-label="<?php esc_attr_e('Select','wpdm-premium-packages'); ?>"><input type="checkbox" class="cboid" value="<?php echo $order->order_id; ?>" name="id[]" aria-label="<?php echo esc_attr( sprintf( __('Select order %s','wpdm-premium-packages'), $order->order_id ) ); ?>"></th>
						...
						<td class="ol-date" data-label="<?php esc_attr_e('Order Date','wpdm-premium-packages'); ?>"><span class="ol-date__stack"><span class="ol-date__d"><?php echo wp_date(get_option('date_format'), $order->date); ?></span><span class="ol-date__t"><?php echo wp_date(get_option('time_format'), $order->date); ?></span></span></td>
							<td class="ol-date" data-label="<?php esc_attr_e('Expire Date','wpdm-premium-packages'); ?>"><span class="ol-date__stack"><span class="ol-date__d"><?php echo wp_date(get_option('date_format'), $order->expire_date); ?></span><span class="ol-date__t"><?php echo wp_date(get_option('time_format'), $order->expire_date); ?></span></span></td>
					...
					<tr class="ol-empty">
						<td colspan="20">
							<div class="ol-empty__inner">
								<div class="ol-empty__icon"><?php echo Icons::get('shopping-cart', 24); ?></div>
								<div class="ol-empty__title"><?php _e('No orders found','wpdm-premium-packages'); ?></div>
								<div class="ol-empty__hint"><?php _e('No orders match the current filters. Try clearing the search or adjusting the date range.','wpdm-premium-packages'); ?></div>
							</div>
						</td>
					</tr>
				<?php } ?>
		...
		<div id="wpdmpp-orders-mobile"></div>
		</div><!-- /.wpdmpp-orders-responsive -->
		<script>
			window.wpdmppOrdersData = <?php echo wp_json_encode( array(
				'items' => $mobile_orders,
				'l10n'  => array(
					...
					'expire'     => __( 'Expire Date', 'wpdm-premium-packages' ),
					...
					'empty'      => __( 'No orders found.', 'wpdm-premium-packages' ),
				),
			) ); ?>;
		</script>
		<script>
			(function () {
				var cfg = window.wpdmppOrdersData || { items: [], l10n: {} };
				if (typeof Vue === 'undefined' || !document.getElementById('wpdmpp-orders-mobile')) return;
				Vue.createApp({
					...
					template:
						...
						'      <div class="wpmo-row"><span class="wpmo-k">{{ L.items }}</span><span class="wpmo-v">{{ o.items }}</span></div>' +
						'      <div class="wpmo-row"><span class="wpmo-k">{{ L.order_date }}</span><span class="wpmo-v">{{ o.order_date }}</span></div>' +
						'      <div class="wpmo-row"><span class="wpmo-k">{{ L.expire }}</span><span class="wpmo-v">{{ o.expire_date }}</span></div>' +
